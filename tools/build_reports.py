...
def build_diabetes():
    ...
    fill_cell(t2.cell(1, 0), "问题解答：", [
        "1. 数据泄漏的处理：本实验的缺失值填补、编码和标准化都只在训练集上拟合，再用于验证集和测试集。这样验证结果才比较可信，不会因为提前看到了测试数据而变得虚高。",
        "2. 模型选择结果：这个数据集是表格型医学数据，树模型比直接上深度学习更合适。随机森林比较稳，LightGBM 的 AUC 最好，CatBoost 也很接近，所以没有只看一个模型。",
        "3. 最终采用的模型：最后更适合采用软投票融合。实验箱端软投票 Accuracy 为 0.9606，Recall 为 0.9612，F1 为 0.9490，阳性样本检出效果比较好。"])
...
def build_chest():
    ...
    fill_cell(t2.cell(1, 0), "问题解答：", [
        "1. PC 端模型结果：PC 机有 CUDA GPU，所以可以使用 vit_b_16。这个模型能学习胸片不同区域之间的全局关系，10 轮训练只用了 33.6 秒，最后 F1 达到 0.9020。",
        "2. 实验箱端模型结果：实验箱主要靠 CPU，继续跑 ViT 不太现实，时间会很长。因此实验箱端使用 convnext_tiny，虽然指标低一些，但能稳定完成迁移验证。",
        "3. 指标解释：这类肺炎筛查任务不能只看 Accuracy，Recall 也很重要。PC 端 Recall 为 0.8679，高于实验箱端 0.8113，说明 PC 端 ViT 漏检肺炎样本的情况相对少一些。"])
    fill_cell(t2.cell(2, 0), "实验小结：", [
