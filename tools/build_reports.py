...
def build_diabetes():
    ...
    fill_cell(t2.cell(1, 0), "问题解答：", [
        "1. 为什么要避免数据泄漏？",
        "答案：本实验的缺失值填补、编码和标准化都只在训练集上拟合，再用于验证集和测试集。这样验证结果才比较可信，不会因为提前看到了测试数据而变得虚高。",
        "2. 为什么没有直接用深度学习，而是选择树模型？",
        "答案：这个数据集是表格型医学数据，字段数量不多，树模型比直接上深度学习更合适。随机森林比较稳，LightGBM 的 AUC 最好，CatBoost 也很接近，所以没有只看一个模型。",
        "3. 最终应该采用哪个模型？",
        "答案：最后更适合采用软投票融合。实验箱端软投票 Accuracy 为 0.9606，Recall 为 0.9612，F1 为 0.9490，阳性样本检出效果比较好。"])
...
def build_chest():
    ...
    fill_cell(t2.cell(1, 0), "问题解答：", [
        "1. 为什么 PC 端使用 Vision Transformer？",
        "答案：PC 机有 CUDA GPU，所以可以使用 vit_b_16。这个模型能学习胸片不同区域之间的全局关系，10 轮训练只用了 33.6 秒，最后 F1 达到 0.9020。",
        "2. 为什么实验箱端没有继续使用 ViT？",
        "答案：实验箱主要靠 CPU，继续跑 ViT 不太现实，时间会很长。因此实验箱端使用 convnext_tiny，虽然指标低一些，但能稳定完成迁移验证。",
        "3. 如何看待 Recall 指标？",
        "答案：这类肺炎筛查任务不能只看 Accuracy，Recall 也很重要。PC 端 Recall 为 0.8679，高于实验箱端 0.8113，说明 PC 端 ViT 漏检肺炎样本的情况相对少一些。"])
    fill_cell(t2.cell(2, 0), "实验小结：", [
        ...
        "模型选型上的体会比较明显。ViT 是更前沿的视觉模型，能够通过自注意力学习不同图像 patch 之间的全局关系，用在胸片任务上有一定优势；但它对算力要求高，如果放到实验箱 CPU 上训练，时间成本会很大。ConvNeXt Tiny 虽然不是 Transformer，但结构轻一些，更适合实验箱端验证可迁移性。最终 PC 端 ViT 的 Accuracy 为 0.9099、F1 为 0.9020，实验箱端 ConvNeXt Tiny 的 Accuracy 为 0.8649、F1 为 0.8515，结果也符合这种硬件和模型差异。",
        "实验中遇到的问题主要有三个。第一是环境判断不能凭感觉，之前误以为本机不能用 CUDA，后来检查 conda 环境才确认 GPU 可用；第二是实验箱没有方便的浏览器和 GitHub 登录环境，结果传回时需要通过文件夹整理或手动拷贝解决；第三是数据量比较小，验证集只有 16 张图像，训练过程中的验证 F1 波动很大，所以最终不能只看某一轮验证结果。通过这次实验，我更理解医学 AI 实验要把模型、数据、硬件和迁移放在一起考虑。"])
    doc.save(out)
    return out
